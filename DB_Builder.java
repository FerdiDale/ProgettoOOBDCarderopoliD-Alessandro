		...
		if(!preesistente)
		{
			try
			{
				...
				stmt.executeUpdate("CREATE FUNCTION NoTavolateSenzaNumeriDiTelefono() RETURNS TRIGGER "
								...
								+ "Counting integer; "
								+ "BEGIN "
								+ "SELECT COUNT(AV.N_Cel) INTO Counting "
								+ "FROM (Elenco_Avventori AS E_A JOIN Tavolata AS TA ON E_A.Id_Tavolata= TA.Id_Tavolata) AS TAV JOIN AVVENTORI AS AV ON AV.N_CID = TAV.N_CID; "
								+ "IF (Counting>0) THEN "
								+ "	DELETE "
								+ "	FROM TAVOLATA AS TA "
								+ "	WHERE TA.Id_Tavolata = NEW.Id_Tavolata; "
